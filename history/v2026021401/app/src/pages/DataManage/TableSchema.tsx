...
export default function TableSchemaPage() {
  ...
  const tableOptions = tables.map(t => ({
    value: t.table_name,
    label: `${t.table_name} (${t.index_count}个索引)`,
  }));
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl">{tableDetail.table_name}</CardTitle>
                  <p className="text-sm text-muted-foreground mt-1">
                    {tables.find(t => t.table_name === tableDetail.table_name)?.index_count || 0} 个索引
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
